 ...
 * @LastEditTime: 2022-07-21 16:58:05
...
int main() {
  ...
  // 指明连接服务器的端口号，htons()用于 converts values
  // between the host and network byte order
  server.sin_addr.s_addr = inet_addr(SERVER_ADDRESS);
  // 指明连接服务器的IP地址
  // 结构sockaddr_in的sin_addr字段用于保存IP地址，sin_addr字段也是一个结构体，sin_addr.s_addr用于最终保存IP地址
  // inet_addr()用于将
  // 形如的"127.0.0.1"字符串转换为IP地址格式
  ...
  while (true) {
    ...
    // sClient指明用哪个连接发送； szMessage指明待发送数据的保存地址
