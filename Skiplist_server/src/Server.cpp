...
// set命令
void setCommand(
    Server *server, Client *client, string key, string &value, bool &flag) {
...
void initServer(Server *&server) {
    ...
    bind(
        server->sListen, (struct sockaddr *)&(server->local),
        sizeof(sockaddr_in));
...
int main() {
    ...
    for (int i = 0; i < MAXCLIENTNUM; i++) {
        ...
        client->sClient = accept(
            server->sListen, (struct sockaddr *)&(client->my_client),
            &iaddrSize);
    ...
    for (int i = 0; i < MAXCLIENTNUM; i++) { pthread_join(hThread[i], NULL); }
    // 关闭套接字
