 ...
 * @LastEditTime: 2022-07-20 19:19:55
 * @FilePath: /Skiplist_cs/Skiplist_server/src/Skiplist.h
 * @Description:
 */
...
#define STORE_FILE "./Disk/dumpFile.txt"
string delimiter = ":";
...
bool Skiplist<K, V>::search_element(K key, V &value) {
  ...
  if (current != nullptr && current->getKey() == key) {
    ...
    // cout << "Found key: " << key << " value: " << current->getValue() <<
    // endl;
    return true;
  }
...
template <typename K, typename V>
void Skiplist<K, V>::get_key_value_from_string(const string &str, string *key,
                                               string *value) {
